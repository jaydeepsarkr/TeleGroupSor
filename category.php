<?php
/**
 * Template Name: Category Taxonomy Archive
 * Description: This template displays posts categorized by standard WordPress categories.
 *
...
if ( $queried_obj instanceof WP_Term ) {
    ...
    $term_desc = term_description( $term_id, 'category' );
...
<main class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 lg:pt-8 mt-2 sm:mt-4 mb-16 font-sans">
    ...
    <section aria-labelledby="yoast_seo_title">
        ...
        $current_term_link  = ( $term_id > 0 ) ? get_category_link( $term_id ) : '';
        ...
        <form id="search-form-second" method="get" action="<?php echo esc_url( $form_action_url ); ?>" class="max-w-xl mx-auto mb-8 sm:mb-10" role="search">
            <div class="relative flex items-center bg-white/95 backdrop-blur-sm rounded-full border border-gray-200/80 shadow-md shadow-gray-200/50 <?php echo esc_attr( $theme_search_focus ); ?> p-1.5 transition-all duration-300">
                ...
                <label for="category-search-input" class="sr-only"><?php esc_html_e( 'Search in this category', 'Telegram_Group_Links' ); ?></label>
                <input
                    type="search"
        ...
        <div class="card-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
                            ...
                            <div class="flex-1 min-w-0 text-left sm:text-center w-full">
                                ...
                                <div class="tags flex flex-wrap items-center sm:justify-center gap-1 mb-2 sm:mb-2.5 max-h-12 overflow-hidden">
                                    ...
                                    <?php foreach ( $category_names as $cat_name ) : ?>
                                        <span class="<?php echo esc_attr( $theme_badge_bg ); ?> text-[10px] font-semibold px-2 py-0.5 rounded-md border"><?php echo esc_html( $cat_name ); ?></span>
                                    <?php endforeach; ?>
                                </div>
            ...
            <?php
                ...
                echo '<p class="col-span-full text-center text-gray-500 py-12">' . esc_html__( 'No posts found in this category.', 'Telegram_Group_Links' ) . '</p>';
            endif;
            ?>
        ...
        <?php
        $total_pages = (int) $blog_posts->max_num_pages;

        if ( $total_pages > 1 ) :
        ?>
            <nav class="pagination flex justify-center items-center gap-1.5 mt-10 font-sans" aria-label="<?php esc_attr_e( 'Category Pagination', 'Telegram_Group_Links' ); ?>">
                <?php
        ...
        <?php if ( ! empty( $term_desc ) ) : ?>
            <div class="term-description mt-10 <?php echo $is_members ? '[&_a]:text-rose-600' : '[&_a]:text-[#229ed9]'; ?>">
