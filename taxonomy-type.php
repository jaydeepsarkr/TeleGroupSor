<?php
/**
 * Template Name: Type Taxonomy Archive
 * Template for displaying 'type' taxonomy archive pages.
 *
...
if ( $queried_obj instanceof WP_Term ) {
    ...
    $term_desc = term_description( $term_id, 'type' );
} elseif ( is_tax() || is_category() || is_tag() ) {
    $term_desc = term_description();
}

// =========================================================================
// 2. RETRIEVE YOAST CUSTOM SEO TITLE FOR TAXONOMY / TERM
// =========================================================================
$dynamic_h1_title = '';

// Method A: Modern Yoast SEO Surface API
if ( function_exists( 'YoastSEO' ) ) {
    $yoast_meta = YoastSEO()->meta->for_current_page();
    if ( $yoast_meta && ! empty( $yoast_meta->title ) ) {
        $dynamic_h1_title = $yoast_meta->title;
    }
}

// Method B: Direct Yoast Taxonomy Meta Array Lookup
if ( empty( $dynamic_h1_title ) && $term_id > 0 ) {
    $taxonomy_meta = get_option( 'wpseo_taxonomy_meta' );
    if ( ! empty( $taxonomy_meta['type'][ $term_id ]['wpseo_title'] ) ) {
        $raw_title = $taxonomy_meta['type'][ $term_id ]['wpseo_title'];
        if ( function_exists( 'wpseo_replace_vars' ) ) {
            $dynamic_h1_title = wpseo_replace_vars( $raw_title, $queried_obj );
        } else {
            $dynamic_h1_title = $raw_title;
        }
    }
}

// Method C: Legacy Yoast Helper Function
if ( empty( $dynamic_h1_title ) && function_exists( 'get_yoast_seo_title' ) ) {
    $dynamic_h1_title = get_yoast_seo_title();
}

// Method D: Standard Term Meta Fallback
if ( empty( $dynamic_h1_title ) && $term_id > 0 ) {
    $term_custom_title = get_term_meta( $term_id, '_yoast_wpseo_title', true );
    if ( ! empty( $term_custom_title ) ) {
        $dynamic_h1_title = function_exists( 'wpseo_replace_vars' ) ? wpseo_replace_vars( $term_custom_title, $queried_obj ) : $term_custom_title;
    }
}

// Method E: Document Title / Term Title Fallback
if ( empty( $dynamic_h1_title ) ) {
    if ( is_tax() || is_category() || is_tag() ) {
        $dynamic_h1_title = single_term_title( '', false );
    } elseif ( is_singular() || is_page() ) {
        $dynamic_h1_title = get_the_title();
    } else {
        $dynamic_h1_title = wp_get_document_title();
    }
}

// Detect 18+ / Members context (consistent with header/single logic)
...
<main class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 lg:pt-8 mt-2 sm:mt-4 mb-16 font-sans">
    ...
    <section aria-labelledby="yoast_seo_title">
        ...
        $current_term_link  = ( $term_id > 0 ) ? get_term_link( $term_id, 'type' ) : '';
        ...
        <form id="search-form-second" method="get" action="<?php echo esc_url( $form_action_url ); ?>" class="max-w-xl mx-auto mb-8 sm:mb-10" role="search">
            <div class="relative flex items-center bg-white/95 backdrop-blur-sm rounded-full border border-gray-200/80 shadow-md shadow-gray-200/50 <?php echo esc_attr( $theme_search_focus ); ?> p-1.5 transition-all duration-300">
                ...
                <label for="type-search-input" class="sr-only"><?php esc_html_e( 'Search in this category', 'Telegram_Group_Links' ); ?></label>
                <input
                    type="search"
                        ...
                        <div class="w-full flex flex-row sm:flex-col items-center sm:items-center flex-1 min-w-0 gap-3.5 sm:gap-0">
                            ...
                            <div class="flex-1 min-w-0 text-left sm:text-center w-full">
                                ...
                                <div class="tags flex flex-wrap items-center sm:justify-center gap-1 mb-2 sm:mb-2.5 max-h-12 overflow-hidden">
                                    <?php foreach ( $type_list as $type_term_name ) : ?>
                                        <span class="bg-slate-100 text-slate-600 text-[10px] font-medium px-2 py-0.5 rounded-md border border-slate-200/60"><?php echo esc_html( $type_term_name ); ?></span>
                                    <?php endforeach; ?>
